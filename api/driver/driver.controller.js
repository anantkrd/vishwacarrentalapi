...
module.exports = {
    getMyTrip: async (req, res) => {
        try {
            ...
            bookingObj = await Booking.find({isDeleted: 'N', $or:[{status: 'confirm'},{status: 'started'}], driverId: userId }).sort({createdAt:-1}).skip(start).limit(perPage);
            if (bookingObj !== null) {
                let rowCount = await Booking.find({isDeleted: 'N', $or:[{status: 'confirm'},{status: 'started'}], driverId: userId }).count();
